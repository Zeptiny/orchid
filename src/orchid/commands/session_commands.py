...
from orchid.llm.providers import discover_provider_models, resolve_model_metadata
...
def _build_model_picker_items(cfg: Config, mode: str = "chat") -> list[PickerItem]:
    ...
    for alias, provider_entry in cfg.providers.items():
        ...
        # ORCHID_DISABLE_MODEL_DISCOVERY for strict configured-only behavior.
        if not models:
            try:
...
async def execute_command(app: App[Any], cmd: str) -> None:
    ...
    match cmd:
        ...
        case "/settings":
            ...
            from orchid.screens.settings import SettingsScreen
        ...
        case "/index-rag":
            ...
            await app.refresh_index_status()
        case "/index-ast":
            from orchid.ast.indexer import index_project as ast_index_project
        ...
        case "/rag clear":
            from orchid.rag.indexer import clear_index
