...
Run standalone: python -m orchid.mcp.example_server
"""
...
async def list_resources() -> list[Resource]:
    ...
    return [
        Resource(
            uri=AnyUrl("example://orchid"),
            name="App Info",
            description="Basic information about the orchid application",
            mimeType="text/plain",
        )
    ]
...
async def read_resource(uri: AnyUrl) -> Iterable[ReadResourceContents]:
    ...
    if str(uri) == "example://orchid":
        return [ReadResourceContents(
            content="orchid is a coding CLI with MCP support",
