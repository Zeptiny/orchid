...
from orchid.config import get_config  # noqa: E402
...
def discover_provider_models(alias: str, force: bool = False) -> list[str]:
    ...
    * Respects `ORCHID_DISABLE_MODEL_DISCOVERY=1/true/yes` for strict
    ...
    * Returns `[]` when the provider has no `base_url`, the endpoint is
    ...
    """
    if os.environ.get("ORCHID_DISABLE_MODEL_DISCOVERY", "").lower() in _DISABLE_DISCOVERY_VALUES:
        log.debug("Model discovery disabled via ORCHID_DISABLE_MODEL_DISCOVERY")
        return []
