...
from orchid.domain.tool import ExecutorResult, Tool
from orchid.llm.dynamic_system_prompt import build_dynamic_system_prompt
from orchid.llm.providers import ProviderResolutionError, qualify_model, resolve_model_ref
from orchid.llm.static_system_prompt import build_static_system_prompt
...
def _patch_litellm_raise_on_model_repetition() -> None:
    ...
    if getattr(CustomStreamWrapper.raise_on_model_repetition, "_orchid_patched", False):
        return
    ...
    _safe_raise_on_model_repetition._orchid_patched = True  # type: ignore[attr-defined]
...
def _is_benign_midstream_litellm_error(exc: MidStreamFallbackError) -> bool:
    ...
    orchid calls ``litellm.acompletion`` directly (no Router), so the
...
async def stream_response(
    ...
    from orchid.tools.skill import build_skill_tool, set_current_allowed_skills
