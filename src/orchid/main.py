...
from orchid.config import HOME_CONFIG_DIR, ConfigManager


def _setup_logging() -> Path | None:
    """Attach a file handler so diagnostics survive the TUI.

    Logs at INFO+ go to ``~/.orchid/logs/orchid.log`` (created on first
    run). Set ``ORCHID_LOG_LEVEL=DEBUG`` for verbose chunk-level traces.
    ...
    try:
        ...
        return None
    log_path = log_dir / "orchid.log"
    level = os.environ.get("ORCHID_LOG_LEVEL", "INFO").upper()
...
def main():
    ...
    if errors:
        print("ORCHID CONFIGURATION ERRORS:", file=sys.stderr)
        for err in errors:
            print(f"  - {err}", file=sys.stderr)
        print(file=sys.stderr)
        print("Fix the errors in ~/.orchid/config.json or .orchid.json and try again.", file=sys.stderr)
        sys.exit(1)
    log_path = _setup_logging()
    if log_path:
        logging.getLogger("orchid.main").info(
            "orchid starting; logs at %s (level=%s)",
            log_path, os.environ.get("ORCHID_LOG_LEVEL", "INFO"),
        )
    try:
        app = Orchid()
        app.run()
