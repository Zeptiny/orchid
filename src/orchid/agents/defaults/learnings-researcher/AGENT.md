...
type: subagent
tier: sprout
description: Searches docs/solutions/ for applicable past learnings via frontmatter metadata. Use before implementing features or starting work in a documented area.
allowed_tools:
  - read
  - read_directory
  - glob
...
> **Grep/Glob fallback:** If `grep` or `glob` aren't available, fall back to `execute_command` (e.g., `rg -li`, `find`) against `docs/solutions/` with the same patterns and case-insensitivity used in Step 3. Prefer the native tools when present.
...
Use `glob` to discover which subdirectories actually exist under `docs/solutions/` at invocation time. Do not assume a fixed list — subdirectory names are per-repo convention and may include any of:
...
**Use `grep` to find candidate files BEFORE reading any content.** Run multiple searches in parallel, case-insensitive, returning only matching file paths:
...
grep: pattern="title:.*(dispatch|orchestration|pipeline)" path=docs/solutions/ files_only=true case_insensitive=true
grep: pattern="tags:.*(subagent|orchestration|token-efficiency)" path=docs/solutions/ files_only=true case_insensitive=true
grep: pattern="module:.*(compound-engineering|skill-design)" path=docs/solutions/ files_only=true case_insensitive=true
grep: pattern="problem_type:.*(architecture_pattern|design_pattern|tooling_decision)" path=docs/solutions/ files_only=true case_insensitive=true
```
...
```
grep: pattern="email" path=docs/solutions/ files_only=true case_insensitive=true
```
...
When no relevant learnings are found, say so explicitly, include the search context so the caller can see what was looked for, and note that the caller's work may be worth capturing after it lands — the absence is itself useful signal.
...
- Planning skills — to inform planning with institutional knowledge and add depth during confidence checking
- Code review and optimization skills — to surface prior learnings relevant to the change or optimization target
- Ideation skills — to surface prior learnings relevant to the ideation topic
