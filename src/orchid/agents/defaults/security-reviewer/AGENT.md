...
type: subagent
tier: crown
description: Reviews code for exploitable vulnerabilities including injection, auth bypass, secrets exposure, insecure deserialization, SSRF, and path traversal. Use when the diff touches auth, public endpoints, user input handling, or permission checks.
allowed_tools:
  - read
  - read_directory
  - glob
