...
type: subagent
tier: crown
description: Reviews code for runtime performance and scalability issues including N+1 queries, unbounded memory growth, missing pagination, hot-path allocations, and blocking I/O. Use when the diff touches database queries, loop-heavy transforms, caching, or I/O-intensive paths.
allowed_tools:
  - read
  - read_directory
  - glob
