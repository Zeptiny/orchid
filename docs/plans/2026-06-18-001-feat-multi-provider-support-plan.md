...
Replace orchid's single-provider config with a `providers` dict (alias, url, api key inline-or-env-ref, optional litellm provider name, per-model metadata overrides) and route models by `alias/model` strings. Per-model metadata (`max_input_tokens`, `max_output_tokens`, `supports_vision`, `mode` — litellm field names verbatim) is resolved by field-level merge of user override → `litellm.get_model_info` registry → text-only fallback, refreshed once at startup. The model picker surfaces metadata and capability badges derived from `supports_vision` + `mode`.

---

## Problem Frame

Orchid is currently locked to a single LLM provider via four top-level `Config` fields (`base_url`, `provider_api_type`, `default_model`, `tier_models` in `src/orchid/config.py:27-37`). The litellm call site (`src/orchid/llm/client.py:325-332`) assembles `cfg.provider_api_type + "/" + model` and passes `base_url=cfg.base_url` — no per-call `api_key`, no per-call routing beyond that one endpoint. Users wanting a second provider (different OpenAI account, local Llama, Anthropic, a corporate proxy) have no path other than editing config and swapping env vars before each launch. The model picker (`src/orchid/commands/session_commands.py:55-67`) hits `base_url + "/models"` for one endpoint's catalog and shows bare IDs with no capability or context-window signal. See origin doc for the full problem narrative and key decisions.
...
- `src/orchid/config.py:25-86` — `Config` dataclass with the four top-level fields to remove (lines 27-37). The `mcp_servers` field (75-86) is the model for `providers` shape.
- `src/orchid/config.py:192-230` — `ConfigManager.load()` merge flow (home → project → env → validate). The duplicated `mcp_servers` deep-merge blocks at 203-208 and 216-221 are the template — and the duplication is a refactor opportunity (extract a shared `_deep_merge_dict_key` helper, with recursive variant for nested `models`).
- `src/orchid/config.py:137-186` — `_validate_config`. The MCP server validation at 163-184 (`isinstance(dict)`, `_MCP_SERVER_NAME_RE`, per-field type-checks, warn-and-skip) is the template for provider validation. The regex `_MCP_SERVER_NAME_RE = r"^[a-z0-9-]+$"` at line 10 forbids `/` already; a similar regex for provider aliases works.
- `src/orchid/config.py:89-106` — `_ENV_MAP`. Three entries must be removed: `ORCHID_BASE_URL`, `ORCHID_DEFAULT_MODEL`, `ORCHID_PROVIDER_API_TYPE` (lines 90-92).
- `src/orchid/config.py:232-251` — `ensure_home_config()` writes `asdict(Config())` to `~/.orchid/config.json` only when the file is absent (line 236). The shipping `Config()` defaults must produce a `providers` section with a `default` provider reproducing today's `https://opencode.ai/zen/go/v1` + `openai` + `mimo-v2.5`.
- `src/orchid/config.py:200-223` — `ConfigManager.load` merge loop only merges keys `in merged` (line 201 and 213), silently dropping unknown keys. Since the project is still in development, no legacy-key tolerance is needed — users will simply write a fresh config matching the new schema when they upgrade.
- `src/orchid/llm/client.py:293-369` — `stream_response`. The litellm call at 325-332 assembles `cfg.provider_api_type + "/" + (model or cfg.default_model)` and passes `base_url=cfg.base_url`. Resolution must produce `(litellm_provider, model_id, base_url, api_key)` and pass `base_url=` + `api_key=` as named kwargs. Resolution happens once before the `while True` loop (model is fixed per session). See §Key Technical Decisions for `base_url` vs `api_base`.
- `src/orchid/llm/models.py:1-24` — `Model` dataclass (bare `id: str`) and `list_models()` httpx call to `cfg.base_url + "/models"`. The function and dataclass are replaced by the new resolved-model object in U2; `list_models()` is removed (R10 drops endpoint discovery).
- `src/orchid/screens/picker.py:1-75` — `OptionPicker(Screen[str])` generic picker; `PickerItem` is a 2-field dataclass (`label`, `id`). `_filter` at 31-33 matches on `label.lower()` and `id.lower()`. **Badges formatted into the `label` string at construction time is the lower-risk path** — keeps picker changes to zero and search behavior intact. The picker is shared by 5 commands (`/switch`, `/delete`, `/model`, `/theme`, `/personality` in `session_commands.py:41,54,67,82,95`).
- `src/orchid/commands/session_commands.py:55-67` — `/model` case. Currently builds `PickerItem(label=m.id, id=m.id)` from `list_models()`. Becomes: iterate configured providers, build PickerItem list from each provider's resolved models, format label with badges and token shorthand.
- `src/orchid/domain/session.py:31-39` — `SessionManager.create()` sets `model=cfg.default_model`. Post-refactor `cfg.default_model` is an `alias/model` string; no code change here.
- `src/orchid/config.py:287-289` — `get_model_for_tier(tier)` returns `cfg.tier_models.get(tier, cfg.default_model)`. Post-refactor both emit `alias/model` strings; no code change here.
- `src/orchid/tools/subagent.py:76` — `model = get_model_for_tier(resolved_tier.value)`; flows into `SubagentManager.spawn(..., model: str | None)`. Unchanged contract post-refactor.
- `src/orchid/rag/embedder.py:19-27` — `Embedder.__init__(model, provider_api_type="openai", embedding_provider="")`. The `provider_api_type` parameter and its 4 call sites (`rag/indexer.py:86,98`; `tools/rag.py:86,178`) are updated in U6 to migrate embeddings to `alias/model` references routed through the providers dict, with `"fastembed/<model_id>"` as a built-in pseudo-provider for local ONNX embeddings.
...
- **New module `src/orchid/llm/providers.py` for resolution + metadata hydration (not in `config.py`):** Keeps the config dataclass focused on shape and validation; the resolution code lives next to the LLM client that consumes it. Module-level functions `resolve_model_ref`, `get_provider`, `resolve_model_metadata` follow the existing module-level-helper pattern (`get_config`, `get_model_for_tier`, `get_current_theme`).
...
- **`list_models()` and the bare `Model` dataclass in `llm/models.py` removed:** R10 drops endpoint discovery; the picker builds from configured providers via the new resolver. The endpoint `/models` call (`src/orchid/llm/models.py:18`) is dead code post-refactor.
...
- Remove `ORCHID_BASE_URL` and `ORCHID_PROVIDER_API_TYPE` from `_ENV_MAP` (lines 90, 92). Keep `ORCHID_DEFAULT_MODEL` since `default_model` remains a top-level field.
- Update `_NON_EMPTY_STRINGS` (line 137) to drop `base_url` and `provider_api_type`; keep `default_model`.
- Drop the `rag_embedding_provider` top-level field (lines 69). Its role is subsumed by `rag_embedding_model`, which now accepts `alias/model` references (including the `fastembed/<model_id>` pseudo-provider handled in U6). Update `rag_embedding_model` default value to `"fastembed/BAAI/bge-small-en-v1.5"` (matches today's effective default of fastembed + the local ONNX model name in `DEFAULT_FASTEMBED_MODEL`). Remove `ORCHID_RAG_EMBEDDING_PROVIDER` from `_ENV_MAP`; keep `ORCHID_RAG_EMBEDDING_MODEL`. Drop the `valid_providers = {"", "openai", "fastembed"}` validation block at 159-161 (no longer relevant).
...
**Patterns to follow:**
- `src/orchid/config.py:163-184` (MCP server validation block — mirror for provider validation)
- `src/orchid/config.py:203-208` and `:216-221` (the merge pattern to refactor and generalize)
...
- Resolution is startup-once (matches R8). Either cache resolved metadata in a module-level dict on first access, OR run once at app startup (`src/orchid/app.py` on_mount) and store on a module-level singleton. Recommend: lazy-cache on first access — simpler test setup. Add a `reset_cache()` function for tests.
...
**Patterns to follow:**
- `src/orchid/config.py:283-315` — module-level helpers like `get_config`, `get_model_for_tier`.
...
- `python -c "from orchid.llm.providers import resolve_model_metadata; print(resolve_model_metadata('default', 'mimo-v2.5'))"` returns a dict with the four fields (without crashing).
- `python -c "from orchid.llm.providers import resolve_embedding_ref; print(resolve_embedding_ref('fastembed/BAAI/bge-small-en-v1.5'))"` returns the pseudo-provider tuple.

---

- U3. **Update `stream_response` to resolve `alias/model` and pass per-call `base_url` + `api_key`**

**Goal:** The litellm call site in `src/orchid/llm/client.py` resolves `alias/model` strings via the new resolver and passes resolved `base_url` and `api_key` per call.
...
**Files:**
- Modify: `src/orchid/config.py` (dataclass defaults — already done in U1; this unit is verification + any missed call sites)
...
- The `Config()` defaults are updated as part of U1: `default_model="default/mimo-v2.5"`, all four `tier_models` values = `"default/mimo-v2.5"`. This unit is mostly verification that every consumer of `default_model` and `get_model_for_tier` works without modification:
  - `src/orchid/domain/session.py:35` — `SessionManager.create()` sets `model=cfg.default_model` — string format change only, no code change.
  - `src/orchid/config.py:289` — `get_model_for_tier` returns `cfg.tier_models.get(tier, cfg.default_model)` — string format change only, no code change.
  - `src/orchid/tools/subagent.py:76` — `model = get_model_for_tier(...)` — passes the string through to the (U3-updated) `stream_response`.
...
**Execution note:** Mostly verification — no new code if U1 and U3 landed cleanly. Add a test that constructs `Config()` default and asserts `cfg.default_model == "default/mimo-v2.5"` and `cfg.tier_models["seed"] == "default/mimo-v2.5"` etc.
...
- `covers F4` — first-run install: `ConfigManager.reset(); ensure_home_config()` writes a `providers.default` entry; `ConfigManager.load().default_model` is `"default/mimo-v2.5"`; `get_model_for_tier("seed")` returns `"default/mimo-v2.5"`.
- Edge case: Custom config with `tier_models.seed = "work-openai/gpt-4o-mini"` — `get_model_for_tier("seed")` returns the string; passes to `stream_response` which resolves it.
- Edge case: Custom config with `tier_models.seed = "missing-alias/gpt-4o"` — value is stored, but resolution at call time raises `ProviderResolutionError` (handled by U3's error classification).
...
**Goal:** The `/model` command in `src/orchid/commands/session_commands.py` builds the picker list from configured providers (not endpoint discovery), with each entry's label showing capability badges and token shorthand derived from resolved metadata.
...
**Files:**
- Modify: `src/orchid/commands/session_commands.py`
- Delete: `src/orchid/llm/models.py` (the `Model` dataclass and `list_models()` — `R10` drops endpoint discovery)
...
- Drop the `from orchid.llm.models import list_models` import at line 8.
- Delete `src/orchid/llm/models.py` entirely — `list_models()` and the bare `Model` dataclass are dead code per R10. Verify no other importers (grep before delete).

**Execution note:** Verify the picker label format renders well in the TUI before finalizing — run `orchid` and open `/model` to check the label width doesn't break the layout.

**Patterns to follow:**
- `src/orchid/commands/session_commands.py:68-82` (`/theme` case — similar structure: build PickerItems from a registry, push OptionPicker).
- `src/orchid/screens/picker.py:31-33` (`_filter` matches on `label.lower()` — badges remain searchable).
...
- Manual: launch `orchid`, run `/model`, see all configured models with badges and token shorthand; selecting one switches the active session's model.
...
**Patterns to follow:**
- `src/orchid/llm/client.py:325-332` (the `litellm.acompletion` call shape U3 establishes — `base_url=` + `api_key=` named kwargs, provider-qualified model string).
...
- `ruff check src/orchid/rag/embedder.py src/orchid/rag/indexer.py src/orchid/tools/rag.py` clean.
- `python -c "from orchid.rag.embedder import Embedder; e = Embedder('fastembed/BAAI/bge-small-en-v1.5'); import asyncio; print(asyncio.run(e.embed_single('hello'))[:5])"` returns a 5-element float list (local ONNX, no network).

---

## System-Wide Impact

- **Interaction graph:** `stream_response` (the central streaming entry point in `src/orchid/llm/client.py`) and `Embedder.embed` (in `src/orchid/rag/embedder.py`) both gain a new dependency on `src/orchid/llm/providers.py`. `SessionManager.create` (`domain/session.py:31-39`) and `get_model_for_tier` (`config.py:287-289`) emit differently-formatted strings but no code change. `/model` command (`commands/session_commands.py:55-67`), the `litellm.acompletion` call site, the `litellm.aembedding` call site, and the 4 `Embedder(...)` construction sites change shape.
...
| `LITELLM_LOCAL_MODEL_COST_MAP=True` setting races other code importing litellm first | Set env var in `src/orchid/main.py` startup before any `import litellm`. Verified by smoke test. |
...
- Code: `src/orchid/config.py` (Config dataclass, ConfigManager, _validate_config, ensure_home_config)
- Code: `src/orchid/llm/client.py` (stream_response litellm call site)
- Code: `src/orchid/llm/models.py` (to be deleted per R10)
- Code: `src/orchid/screens/picker.py` (OptionPicker, PickerItem)
- Code: `src/orchid/commands/session_commands.py` (/model command)
- Code: `src/orchid/rag/embedder.py`, `src/orchid/rag/indexer.py`, `src/orchid/tools/rag.py` (embeddings routing migration)
