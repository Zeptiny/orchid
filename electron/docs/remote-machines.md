...
Provider credentials are the exception: the daemon is headless, so the encrypted credential vault is **unavailable** — its plain-Node storage adapter reports encryption unavailable. Connections are configured from the app like on the local machine (create/update/edit all route to the driven machine's host), but API-key storage is capability-gated: the daemon does not declare `providers.vault-writes`, so submitting an API key — or creating/editing a connection with **API-key** authentication — fails closed with the typed `UNSUPPORTED_ON_HOST` error steering you to the alternative. Provider connections on a remote use **environment-variable references**: the variable name is stored in the remote's `~/.orchid` (the same mechanism the provider live smoke tests use); its value is resolved from the daemon's environment only at request time. The connection wizard reflects this automatically — the remote's overview reports secure storage unavailable, the API-key field is replaced with guidance, and environment-auth connections work end-to-end. Reads, model listing, and model picking all work against a remote; only API-key storage does not. (Draft live discovery before a connection exists — `providers:discover_draft_models` — stays local-only because it resolves a pasted key or env value against this machine's drivers.)
...
| `METHOD_NOT_FOUND` | The remote agent is older than the app and lacks a method the app invoked (e.g. `providers.create` after a provider-configuration update). | Update `orchid-agent` on the remote to the app's version (new methods are additive, but both sides must run the same build). |
| `UNSUPPORTED_ON_HOST` | A host-routed capability the daemon does not declare was requested against a remote (e.g. storing an API key or creating an API-key-auth connection, the native folder picker, revealing a definition file on the host). | Configure the remote's providers with environment-variable authentication; bind remote workspaces by typed path. |
...
- Provider vault writes (API-key storage) are unsupported on remotes; connections are configured with environment-variable references instead (create/update/edit route to the driven machine).
