/**
 * Config zod schemas — shared source of truth for config types and validation.
 *
 * Hoisted from main/config/schema.ts so protocol-level consumers
 * (shared/host/protocol.ts) can validate `config.get`/`config.get_home`
 * results and config-boundary request payloads without a shared→main import
 * (layer-boundary fix); main/config/schema.ts re-exports everything below
 * for its existing main-process import sites.
 *
 * NOTE: this module is intentionally NOT part of the sandboxed-preload graph
 * (it uses node:path for the absolute-path refinement on
 * `default_project_dir`); keep request schemas that preload must bundle in
 * ./ipc-schemas.ts instead.
 */
import * as path from 'node:path';
import { z } from 'zod';
import { PERMISSION_MODE_VALUES } from './permission';
import { modelSelectionSchema } from './provider';
import { remoteMachineRecordSchema } from './machine';
...
// ---------------------------------------------------------------------------
// Config-boundary request payloads
// ---------------------------------------------------------------------------
//
// These structurally depend on configSchema (the known-top-level-key allow
// list and the compaction partial shapes), so they live here rather than in
// ./ipc-schemas.ts — keeping the preload graph free of node:path (see the
// module header). main/ipc/payload-schemas.ts re-exports them.
