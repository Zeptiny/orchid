...
- **The fix would make the code worse** -- it violates a project rule in AGENTS.md, adds dead defensive code, suppresses errors that should propagate, introduces premature abstraction, or restates code in comments -> verdict: `declined`, citing the specific harm.
