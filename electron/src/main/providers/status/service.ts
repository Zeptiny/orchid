...
function unavailableObservation(providerId: string, now: Date, error: ProviderStatusError): ProviderStatusObservation {
  return {
    providerId,
...
export class ProviderStatusService {
  ...
  refresh(source: ProviderStatusSource, options: StatusRefreshOptions = {}): Promise<StatusRefreshResult> {
    const existing = this.inFlight.get(source.providerId);
    if (existing) return existing;

    const refresh = this.refreshOnce(source, options).finally(() => {
      this.inFlight.delete(source.providerId);
    });
    this.inFlight.set(source.providerId, refresh);
    return refresh;
  }

  private async refreshOnce(source: ProviderStatusSource, options: StatusRefreshOptions): Promise<StatusRefreshResult> {
    ...
    const now = this.now();
    const cached = this.cache.get(source.providerId);
    const retryAt = retryAfterAt(cached);
    if (retryAt !== undefined && retryAt > now.getTime()) {
      const observation = cached ?? unavailableObservation(source.providerId, now, {
        kind: 'rate-limited',
...
export class ProviderStatusScheduler {
  ...
  start(sources: readonly ProviderStatusSource[]): void {
    ...
    for (const source of sources) {
      if (this.timers.has(source.providerId)) {
      ...
      timer.unref?.();
      this.timers.set(source.providerId, timer);
    }
  }
