...
import { isOutsidePopoverRoot } from '../ui/popover-listbox-logic';
...
export const MachineSwitcher = memo(function MachineSwitcher({
  ...
  useEffect(() => {
    ...
    const onPointerDown = (event: MouseEvent) => {
      if (isOutsidePopoverRoot(rootRef.current, event.target as Node)) {
        close();
      }
    };
  ...
  return (
    <div ref={rootRef} className="dropdown dropdown-end">
      <button
        ref={triggerRef}
        type="button"
        className="btn btn-ghost btn-xs orchid-machine-switcher-trigger min-w-0 gap-1.5 px-1.5"
        aria-haspopup="menu"
        aria-expanded={open}
        aria-label={`Machine: ${machines.activeMachineLabel} (${activeStatus.state})`}
      ...
      {open && (
        <div role="menu" aria-label="Machines" className="dropdown-content z-50 mt-1 w-72">
